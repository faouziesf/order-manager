...
@section('title', 'Integrations PrestaShop')

@section('css')
<style>:root { --intg-color-1: #df0067; --intg-color-2: #ff4081; }</style>
...
<div class="om-page-header">
    <div>
        <h1 style="font-size:24px; font-weight:800; color:var(--om-text); margin:0;">
            <i class="fas fa-shopping-bag" style="color:#df0067; margin-right:8px;"></i>Integrations PrestaShop
        </h1>
        <p style="color:var(--om-text-light); margin:4px 0 0; font-size:14px;">Connectez vos boutiques PrestaShop pour importer automatiquement les commandes</p>
    </div>
    <div style="display:flex; gap:8px; flex-wrap:wrap;">
        <button class="om-btn om-btn-primary" style="background:#df0067; border-color:#df0067;" data-bs-toggle="modal" data-bs-target="#addIntegrationModal">
        ...
        <a href="{{ route('admin.prestashop.sync') }}" class="om-btn om-btn-success">
...
<h5 style="font-weight:700; color:var(--om-text); margin-bottom:16px;">
    <i class="fas fa-list" style="margin-right:8px; color:#df0067;"></i>Integrations configurees
</h5>
...
<div class="om-card" style="text-align:center; padding:48px 32px;">
    <div class="om-empty">
        ...
        <h3>Aucune integration PrestaShop</h3>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header intg-modal-header">
                <h5 class="modal-title"><i class="fas fa-shopping-bag" style="margin-right:8px;"></i>Nouvelle integration PrestaShop</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('admin.prestashop.store') }}" method="POST" id="prestashop-form">
                @csrf
                <div class="modal-body">
                    <div class="conn-result" id="connection-result"></div>
                    <div class="om-form-group" style="margin-bottom:16px;">
                        <label class="om-form-label">URL de la boutique PrestaShop</label>
                        <input type="url" class="om-form-input @error('shop_url') is-invalid @enderror" id="shop_url" name="shop_url" value="{{ old('shop_url') }}" required placeholder="https://votreboutique.com">
                        @error('shop_url')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="om-form-group" style="margin-bottom:16px;">
                        <label class="om-form-label">Cle Web Service PrestaShop</label>
                        <input type="text" class="om-form-input @error('api_key') is-invalid @enderror" id="api_key" name="api_key" value="{{ old('api_key') }}" required placeholder="XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX">
                        @error('api_key')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        <small style="color:var(--om-text-light); font-size:12px; margin-top:4px; display:block;">Cle generee dans les parametres Web Service de PrestaShop</small>
                    </div>
                    ...
                    <div class="help-box">
                        <h6><i class="fas fa-info-circle" style="margin-right:8px;"></i>Comment obtenir votre cle API</h6>
                        <ol>
                            <li>Connectez-vous au back-office PrestaShop</li>
                            <li>Allez dans <strong>Parametres Avances > Webservice</strong></li>
                            <li>Activez le Webservice si necessaire</li>
                            <li>Cliquez sur <strong>"Ajouter une nouvelle cle"</strong></li>
                            <li>Configurez les permissions: <strong>orders, products, customers, addresses</strong></li>
                            <li>Enregistrez et copiez la cle dans le champ ci-dessus</li>
                        </ol>
                    </div>
                    <div class="info-box">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="om-btn om-btn-primary" style="background:#df0067; border-color:#df0067;"><i class="fas fa-save"></i> Ajouter</button>
                </div>
            </form>
        </div>
    </div>
...
function testConnection() {
    ...
    if (!url || !key) { showConn('error', 'Remplissez tous les champs.'); return; }
    btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Test...';
    fetch('{{ route("admin.prestashop.test-connection") }}', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken },
        body: JSON.stringify({ shop_url: url, api_key: key })
...
function toggleIntegration(id, cb) {
    ...
    fetch('{{ route("admin.prestashop.index") }}/toggle/' + id, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken },
...
function deleteIntegration(id, name) {
    if (confirm('Supprimer l\'integration avec ' + name + ' ?'))
        window.location.href = '{{ route("admin.prestashop.index") }}/delete/' + id;
}

function refreshStats() {
    fetch('{{ route("admin.prestashop.stats") }}').then(r => r.json()).then(d => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('prestashop-form').addEventListener('submit', function() {
    ...
    document.getElementById('addIntegrationModal').addEventListener('hidden.bs.modal', function() {
        document.getElementById('prestashop-form').reset();
