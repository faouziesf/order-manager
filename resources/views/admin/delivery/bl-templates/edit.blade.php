...
@section('title', 'Modifier Template BL')
                ...
                <div>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Modifier "{{ $blTemplate->template_name }}"</li>
                        </ol>
                    </nav>
                    <h1 class="h3 mb-1">
                        <i class="fas fa-edit text-primary me-2"></i>
                        Modifier Template BL
                    </h1>
                    <p class="text-muted mb-0">Modifiez le modèle d'étiquette "{{ $blTemplate->template_name }}"</p>
                </div>
                <div>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="template_name" name="template_name" 
                                           value="{{ $blTemplate->template_name }}" required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="carrier_slug" name="carrier_slug">
                                        ...
                                        @foreach($carriers as $slug => $name)
                                            <option value="{{ $slug }}" {{ $blTemplate->carrier_slug == $slug ? 'selected' : '' }}>
                                                {{ $name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    <label for="logo" class="form-label">
                                        {{ !empty($blTemplate->layout_config['logo']['path']) ? 'Remplacer le logo' : 'Ajouter un logo' }}
                                    </label>
                            ...
                            <div class="card-body p-0">
                                <div class="template-designer" id="templateDesigner" ondrop="dropField(event)" ondragover="allowDrop(event)">
                                    ...
                                    <div class="design-area" id="designArea">
                                        <!-- Les champs existants seront chargés ici -->
                                    </div>
                                </div>
                            </div>
                        </div>
...
let fieldCounter = 0;
let templateConfig = @json($blTemplate->layout_config);
...
$(document).ready(function() {
    console.log('Éditeur de template BL chargé');
    ...
    $('#logo').on('change', function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                templateConfig.logo = templateConfig.logo || {};
...
function loadExistingFields() {
    ...
    if (templateConfig.fields) {
        Object.keys(templateConfig.fields).forEach(fieldId => {
            ...
            fieldElement.style.left = fieldConfig.x + 'px';
            fieldElement.style.top = fieldConfig.y + 'px';
            fieldElement.style.width = fieldConfig.width + 'px';
            fieldElement.style.height = fieldConfig.height + 'px';
            fieldElement.style.fontSize = fieldConfig.fontSize + 'px';
            fieldElement.style.fontWeight = fieldConfig.fontWeight;
            fieldElement.style.color = fieldConfig.color;
            fieldElement.style.textAlign = fieldConfig.align;
            
            fieldElement.innerHTML = `
                <div class="field-label">${fieldConfig.label}</div>
                <div class="field-content">${getSampleValue(fieldConfig.key)}</div>
            ...
            // Ajouter au design
            designArea.appendChild(fieldElement);
            
            // Mettre à jour le compteur
            const fieldNumber = parseInt(fieldId.replace('field_', ''));
            if (fieldNumber > fieldCounter) {
                fieldCounter = fieldNumber;
            }
        });
    }
    
    // Afficher un message si aucun champ
    if (!templateConfig.fields || Object.keys(templateConfig.fields).length === 0) {
        designArea.innerHTML = `
...
function addFieldToDesign(fieldData, x, y) {
    ...
    // Effacer le message par défaut si présent
    const designArea = document.getElementById('designArea');
    if (designArea.querySelector('.text-center.text-muted')) {
        designArea.innerHTML = '';
    }
    ...
    // Initialiser templateConfig.fields si nécessaire
    if (!templateConfig.fields) {
...
function makeFieldDraggable(element) {
    ...
    function drag(e) {
        ...
        if (templateConfig.fields && templateConfig.fields[element.id]) {
...
function updateFieldProperty(fieldId, property, value) {
    if (!templateConfig.fields || !templateConfig.fields[fieldId]) return;
...
function removeField(fieldId) {
    ...
    if (element) {
        ...
        // Afficher le message par défaut si plus de champs
        const designArea = document.getElementById('designArea');
...
function generatePreview() {
    ...
    if (templateConfig.logo && templateConfig.logo.enabled && templateConfig.logo.path) {
        html += `<img src="${templateConfig.logo.path}" 
                     style="position: absolute; top: 10px; left: 10px; width: ${templateConfig.logo.width || 100}px; height: ${templateConfig.logo.height || 50}px;">`;
    }
...
function saveTemplate() {
    ...
    fetch('{{ route("admin.delivery.bl-templates.update", $blTemplate) }}', {
        method: 'POST',
        headers: {
